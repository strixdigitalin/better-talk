...
import { setUserId,setName,setAge,setGender,setFreeSession,setLocation,setMedHistory,setQualification, setUpcomingApp } from '../reducers/userReducer';
...
export const postUserAsync = createAsyncThunk(
  'users/postUserAsync',
  ({name, qualification, location, age, gender, medHistory, freeSession, upcomingApp,dispatch,sessions, minutes}) => {
    return axios
...
export const postAppointmentAsync = createAsyncThunk(
  ...
  ({from, to, fromName, acceptStatus, startStatus, time, appointmentType, dispatch}) => {
    return axios
    ...
    .then(function (response) {
        console.log('response: postAppointmentAsync message data._id', response.data.data._id);
